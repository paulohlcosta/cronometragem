...
//-----------------------------------------------------------MARCACAO DO TEMPO DE PISTA
  ...
  if ((sequencia < 4) or (sequencia > 6)) digitalWrite(TXENABLE, HIGH);
...
//--------------------------------------------------------------- ATIVAMENTO DO GATILHO
  //se acabou de acender as 4 luzes vamos entrar em espera
  if ((automatica == true) and (sequencia == 0) and digitalRead(STAGED) 
  and digitalRead(STAGEE) and digitalRead(PRESTAGED) and digitalRead(PRESTAGEE)) { 
  //o contador *espera* controla todos as condicoes:
    if (espera == 0) em_espera_desde = millis() - 1;
    espera = millis() - em_espera_desde; //diferenca de horas = variavel usada adiante
  } else { //
    espera = 0; //se ambos os carros nao estao alinhados, zera a contagem
  }
  
  if(((digitalRead(GATILHO) == LOW) and !tem_queima_d and !tem_queima_e)   
  or   ((automatica == true) and sequencia == 0 and espera > 2000
  and !tem_queima_d and !tem_queima_e)){
    sequencia = 1; //coloca em STAGE DE AMBOS
    largada();
  }
